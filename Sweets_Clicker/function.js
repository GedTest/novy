...
function Clicking(Player, ctx) {
    ...
    // candyStick item
    canvas.addEventListener("click", () => {
        if (((x > (candyStick.x - 20)) && (y > candyStick.y)) && ((x < ((candyStick.x - 20) + candyStick.width + 40)) && (y < (candyStick.y + candyStick.height + 60)))) {
            if (Player.counter >= candyStick.cost) {
                Player.counter -= candyStick.cost;
                candyStick.counter = 1;
                 setInterval(() => { mainPanel.image.src = 'img/Pernicek_w_candyStick.png'; }, 20);
            }
        }
    });

/* function: when hover over item shows text
    canvas.addEventListener("mouseover", () => {
        if (((x > (candyStick.x - 20)) && (y > candyStick.y)) && ((x < ((candyStick.x - 20) + candyStick.width + 40)) && (y < (candyStick.y + candyStick.height + 60)))) {
            alert(`x: ${x} y: ${y}`);
            ctx.fillText(`Název`, 1180, 100);
            ctx.fillText(`Produkce perníčků + 100/s`, 1180, 120);
        };
    });
*/
    // cepice item
    canvas.addEventListener("click", () => {
        if (((x > cepice.x - 10) && (y > 100)) && ((x < (cepice.x - 10 + cepice.width + 20)) && (y < (100 + cepice.height + 30)))) {
            if (Player.counter >= cepice.cost) {
                ...
                cepice.counter = 1;
                 setInterval(() => { mainPanel.image.src = 'img/Pernicek_w_hat.png'; }, 20);
            }
        }
    });
    ...
    canvas.addEventListener("click", () => {
        if (((x > svetylka.x + 50) && (y > 100)) && ((x < (svetylka.x + 50 + svetylka.width - 100)) && (y < (100 + svetylka.height + 30)))) {
            if (Player.counter >= svetylka.cost) {
                ...
                svetylka.counter = 1;
                 setInterval(() => { mainPanel.image.src = 'img/Pernicek_w_lights.png'; }, 20);
            }
        }
    });
}
/*
// -------H-I-T--B-O-X-------
function Hitbox(objX, objY, objHeight, offsetX, offsetY) {
    var x = 0;
    var y = 0;
    window.onload = init;
    ...
    if (((x > objX) && (y > offsetY)) && ((x < (objX + offsetX)) && (y < (offsetY + 75))))
        return true;
    else
        return false;
}
*/
function Buy(obj) { // Buy an item and start producing score per second
    if (Player.counter >= Math.round(obj.cost * (1.15 ** obj.counter))) {
        Player.counter -= Math.round(obj.cost * (1.15 ** obj.counter));
        obj.counter++;
        // Timer; every sec +1 to score
        setInterval(() => { cursor.drawMainPanel(ctx); }, 1000);
    }
}
...
setInterval(() => {
    game.prerender();
        Player.counter += ((cursor.multiplier * cursor.counter) + 
        (elf.multiplier * elf.counter) + 
        (darek.multiplier * darek.counter) + 
        (sanky.multiplier * sanky.counter) + 
        (stromecek.multiplier * stromecek.counter) + 
        (candyStick.multiplier * candyStick.counter) + 
        (cepice.multiplier * cepice.counter) + 
        (svetylka.multiplier * svetylka.counter));
}, 1000);